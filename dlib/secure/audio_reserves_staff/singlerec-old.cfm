...
<tr><th colspan="3">Annotation for class #title#:</th></tr>
<cfif url.ed is "yes" or structKeyExists(url,"edannot")>
...
<cfif session.acctype neq "admin"><input type="hidden" name="edannot"><input type="submit" value="Save"></cfif>
<cfelse>
<tr><td colspan="3"> 
#annotation#<cfif session.acctype neq "admin"></td></tr>
...
<tr><td colspan="10"><center><b>Annotation for class 
...
<select name="id"><cfloop query="classes"><cfif session.acctype eq "admin" or UCLA_ID eq session.user or ListContainsNoCase(TAs,session.user)><option value="#id#" <cfif annots.id eq classes.id>selected</cfif>>#title#</option></cfif></cfloop></select></b></center>
<a name="ItemAnnot">
<textarea wrap="virtual" 
			name="itemannotation" 
			rows="3" 
...
<cfelse>
#annots.title#</b></center>#annots.itemannotation#
</cfif>
<cfif url.ited eq "yes">
	<cfif (srch2.itemid[srch2.currentRow] eq annots.itemid and url.itemid eq annots.itemannotid)><center><input type="submit" value="Save"></center></cfif>
<cfelseif url.ed eq "no">
	...
	<center>
	<a href="singlerec.cfm?ed=no&ited=yes&id=#variables.workid#&itemid=#annots.itemannotid###ItemAnnot" class="go2">EDIT</a>&nbsp;/&nbsp;<a href="deleteitem.cfm?itemid=#annots.itemannotid#&workid=#workid#&annotate" class="go2" onClick="return confirm('Delete this annotation?')">DELETE</a></center>
	</cfif>
</cfif>
</td>
</tr>
