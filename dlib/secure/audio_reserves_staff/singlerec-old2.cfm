...
select itemannot.*, classes.title, classes.classid, classes.TAs, instructors.UCLA_ID from itemannot, classes, link, instructors
...
order by ordr,volume,disc,side,cast(track as int),caption
</cfquery>
...
<tr><th colspan="3">Class &quot;<a href="classrec.cfm?t=view&classid=#classid#&listadd=no" class="go2">#title#</a>&quot;</th></tr>
...
<table>
<cfloop query="srch2">
<tr><td><input type="checkbox" name="items#form_num#" value="#itemid#" <cfif not ListContains(classes.items[classes.currentRow],srch2.itemid)>checked="checked"</cfif>>Track ###srch2.currentRow#: #caption#</td></tr>
</cfloop>
<tr><td colspan="2">&nbsp;</td></tr>
<tr><td colspan="2"><input type="button" value="Clear Selections" onClick="for(var x=0;x<#srch2.recordCount#;x++){document.getElementsByName('items#form_num#')[x].checked=false;}">
<input type="button" value="Select All" onClick="for(var x=0;x<#srch2.recordCount#;x++){document.getElementsByName('items#form_num#')[x].checked=true;}">
<cfif session.acctype neq "admin">
...
<cfloop query="srch2">
<cfif not ListContains(classes.items[classes.currentRow],srch2.itemid)><cfset selectedTracks=ListAppend(selectedTracks," Track###srch2.currentRow#: #caption#")></cfif>
</cfloop>
#selectedTracks#
...
<tr>
	<cfif ited eq "yes" and (srch2.itemid eq url.itemid or structKeyExists(url,"editall")) and not structKeyExists(url,"annotate")>
		...
		<td><input type="text" name="caption#form_num#" value="#replace(srch2.caption,'"',"&quot;","all")#" size="60" maxlength="100"></td>
...
<select name="id"><cfloop query="classes"><cfif session.acctype eq "admin" or UCLA_ID eq session.user or ListContainsNoCase(TAs,session.user)><option value="#id#" <cfif annots.id eq classes.id>selected</cfif>>#title#</option></cfif></cfloop></select></b></div>
<a name="ItemAnnot">
<textarea wrap="virtual" 
			name="itemannotation" 
			rows="3" 
...
<cfelse>
#annots.title#</b></div>#annots.itemannotation#
</cfif>
...
<tr>
<th colspan="10">Annotation for class <select name="id"><cfloop query="classes"><cfif session.acctype eq "admin" or UCLA_ID eq session.user or ListContainsNoCase(TAs,session.user)><option value="#id#">#title#</option></cfif></cfloop></select></th></tr><tr><td colspan="10">
<textarea wrap="virtual" 
			name="itemannotation" 
			rows="3" 
			cols="69" 
