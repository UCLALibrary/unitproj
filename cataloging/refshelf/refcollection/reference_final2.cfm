...
<cfif URL.search_query EQ "">
  <cflocation url="index.cfm" /> 
</cfif>

<cfif URL.boolSearch NEQ "phrase">

    <cfset CommaDelimitedList = replace(URL.search_query, " ",",","all")> 
	<cfset CommaDelimitedList = replace(CommaDelimitedList, "?", "%","all")>
	<cfset CommaDelimitedList = replace(CommaDelimitedList, "'", "_","all")>	
...
<cfelse>
  ...
  <cfquery datasource="#DSN#" name="qReference" cachedwithin="#CreateTimeSpan(0,0,5,0)#">
    SELECT Author, Date, nDate, Title, Location, Room
    FROM CatCenterRef
...
<table border="0" width="100%" cellpadding="10" cellspacing="2" style="Font-size:12px">
...
<!--- 	  NAVIGATION GOES HERE   --->
  ...
  <tr>
	...
	<td width="40%">#qReference.Title#</td>
	<td width="10%" align="center">#qReference.Date#</td>
	<td width="10%" align="center">#qReference.Location#</td>
	<td width="10%" align="center">#qReference.Room#</td>
  </tr>
  </cfoutput>
  
  </cfif>
</cfif>
  <cfif IsDefined("URL.submit_button") AND (qReference.RecordCount LT 1 OR URL.search_query EQ "")>
	<br><hr>
  	<i>Your search returned 0 results.</i>
	<br><hr>
  </cfif>
</table>
