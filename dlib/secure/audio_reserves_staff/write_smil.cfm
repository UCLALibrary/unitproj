...
<cfoutput query="smilBase">
	...
	<cfset smilDirectory=getDirectoryFromPath(CF_TEMPLATE_PATH) & "..\..\audio_reserves_staff\smil\">
	<cfset baseAddress=trim(smilBase.baseAddress)>
	<cfif smilBase.ramlink neq "">
		<cfset metaF=trim(smilBase.ramlink)&".smil">
		<cfset metaFilename=getDirectoryFromPath(CF_TEMPLATE_PATH) & "..\..\audio_reserves_staff\smil\" & metaF>
	<cfelse>
		<cfset metaF=0>
...
select itemid,ordr,volume,disc,side,track,parfile,caption
	from items
	where workid=#url.workid#
...
<cfif metaF neq 0>
	...
	<cffile action="append" file="#metaFilename#" output='<meta name="base" content="#srv9#/"/>' addnewline="yes">
...
<cfoutput query="smilFiles">
	...
	<cffile action="append" file="#fn2#" output='<meta name="base" content="#srv9#/"/>' addnewline="yes">
	...
	<cffile action="append" file="#fn2#" output='<audio src="#fn1#" title="#variables.caption#"/>' addnewline="yes">
	<cfif parfile neq "">
