...
<cfparam name="title_error" default="Please check in Voyager.">
...
<cfif IsDefined("URL.get_title")>
	<cfif URL.address NEQ "" AND URL.address NEQ "[required]" and URL.address NEQ address_error>
		...
		<cfscript>
			isSRLF = Find("from the menu bar", cfhttp.FileContent);
			if (isSRLF GT 0) {
				URL.title = "This record has SRLF holdings.";
				titleClass="inputbox"; }
			else { 
				URL.title = isSRLF;
				titleClass = "inputboxerror";
			}
			URL.summary = cfhttp.FileContent;
		</cfscript>
		</cfif>
	<cfelse>
		<cfscript>
...
<tr><td valign="middle" align="center">
	<form action="#CGI.SCRIPT_NAME#" method="get" name="eres" id="eres">
			...
			<input type="hidden" name="zrecipient" value="rhunter@library.ucla.edu">
	...
	<table class="webtool" id="webtool_form" cellpadding="2">
			...
			<tr>
				<td></td>
				<td><input class="button" type="button" name="submit_button" value="Send to Cataloging">
					<input class="button" type="button" value="Clear" onclick="window.location.href='http://unitdev.library.ucla.edu/cataloging/test/ZAS_tool.cfm'">
				</td>
			</tr>
	</table>
	</form>
...
Problems? Please send us an email: <a href="mailto:rhunter@library.ucla.edu?subject=Webtool problems">rhunter@library.ucla.edu</a>	
</span>
</td></tr>
</table>
</center>
