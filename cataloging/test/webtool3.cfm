<cfif Find("164.67",CGI.REMOTE_ADDR) GT 0>
...
<style type="text/css">
	.webtool {border-color: #5075a1; border-style: outset; border-width: medium; background-color: #5075a1; width: 70%; font-family: arial;}	
	.webtool td {border-color: #aaaaaa; border-width: 1px; border-style: none;}
	.inputbox {background-color: #ffffcc; border-color: #5075a1; border-style: inset; height: 26px; padding: 2px;}
	.inputboxerror {background-color: pink; border-color: #5075a1; border-style: inset; height: 26px; padding: 2px;}
	.radio {background-color: #5075a1; border-color: #5075a1; border-style: none; color: #ffffff;}	
	.button {border-color: #5075a1; border-width: medium; border-style: outset; height: 26px; padding: 2px; color: #ffffff; background-color: #5075a1;}	
...
<tr><td valign="middle" align="center">
	...
	<table class="webtool" id="webtool_form" cellpadding="5">
			<tr><td colspan="2" align="center"><h2>REQUEST TO CATALOG<br>
											   Freely-Available Web Sites</h2>
											   <hr size="1" width="50%" color="#ffffff">										   
			</td></tr>		
			<tr><td align="right" class="leftcol">Your name:</td><td><input class="inputbox" type="text" id="name" name="name" value="<cfoutput>#URL.name#</cfoutput>" size="68" onfocus="checkValue(this.id)" onblur="checkValue_input(this.id)"></td></tr>
			<tr><td align="right" class="leftcol">Your email:</td><td><input class="inputbox" type="text" id="email" name="email" value="<cfoutput>#URL.email#</cfoutput>" size="68" onfocus="checkValue(this.id)" onblur="checkValue_input(this.id)"></td></tr>
			<tr><td align="right" class="leftcol">Title:</td><td><input class="<cfoutput>#titleClass#</cfoutput>" type="text" id="title" name="title" value="<cfoutput>#URL.title#</cfoutput>" size="68" onfocus="checkValue(this.id)" onblur="checkValue_input(this.id)"></td></tr>
			<tr><td align="right" class="leftcol">URL:</td><td><input class="<cfoutput>#addressClass#</cfoutput>" type="text" id="address" name="address" value="<cfoutput>#URL.address#</cfoutput>" size="68" onfocus="checkValue(this.id)" onblur="checkValue_input(this.id)">
															   <input class="button" type="submit" name="get_title" value="Get Title"></td></tr>
																   <tr>
																	<td></td>
																	<td>
			...
			<tr><td align="right" class="leftcol">Resource type:</td><td><input class="radio" type="radio" name="type" value="book" <cfif URL.type EQ "book">checked</cfif>>Book<br>
										   ...
										   <input class="radio" type="radio" name="type" value="database" <cfif URL.type EQ "database">checked</cfif>>Database</td></tr>
			<tr><td align="right" class="leftcol">Priority:</td>
				<td>
			...
			<tr>
				...
				<td>
					<textarea class="textarea" id="summary" name="summary" rows="10" cols="52" onfocus="checkValue_summary(this.id)"><cfoutput>#URL.summary#</cfoutput></textarea>
				</td>
			</tr>
			<tr><td></td><td><input class="button" type="button" name="submit_button" value="Send to Cataloging" onclick="validateForm()">
							 <input class="button" type="button" value="Clear" onclick="window.location.href='http://unitdev.library.ucla.edu/cataloging/test/webtool3.cfm'"></td></tr>
	</table>
	</form>
</td></tr>
</table>
...
<script type="text/javascript">
    ...
    function validateForm() {
		...
		if (numErrors == 0) {
			...
			alert("Please fix the errors (in pink) before resubmitting this form.");
		}
	}
</script>
